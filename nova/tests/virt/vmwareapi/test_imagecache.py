...
class ImageCacheManagerTestCase(test.NoDBTestCase):
    def setUp(self):
        ...
        self._session.i_get_vim.return_value = None
    ...
    def test_timestamp_cleanup(self):
        ...
        ) as (_get_timestamp, _delete_file):
            self.exists = False
            self._imagecache.timestamp_cleanup('fake-instance',
                    'fake-dc-ref', 'fake-ds-browser', 'fake-ds-ref',
                    'fake-ds-name', 'fake-ds-path')
            self.assertEqual(0, _delete_file.call_count)
            self.exists = True
            self._imagecache.timestamp_cleanup('fake-instance',
                    'fake-dc-ref', 'fake-ds-browser', 'fake-ds-ref',
                    'fake-ds-name', 'fake-ds-path')
            _delete_file.assert_called_once_with(self._session,
                    'fake-instance', 'fake-ds-path/ts-2012-11-22-12-00-00',
                    'fake-dc-ref')
    ...
    def test_get_base(self):
        datastore = {'name': 'ds'}
        ds_path = self._imagecache._get_base(datastore)
    ...
    def test_list_base_images(self):
        ...
        with contextlib.nested(
            ...
            ds_path = self._imagecache._get_base(datastore)
            images = self._imagecache._list_base_images(ds_path, datastore)
            originals = set()
    ...
    def test_age_and_verify_cached_images(self):
        ...
        def fake_file_delete(session, instance, path, dc_ref):
            self.assertEqual('fake-ds-path/fake-image-3', path)

        def fake_timestamp_cleanup(instance, dc_ref, ds_browser,
                                   ds_ref, ds_name, path):
        ...
        with contextlib.nested(
            ...
            self._imagecache._age_and_verify_cached_images('fake-context',
                    datastore, dc_info, 'fake-ds-path')
            self.assertEqual(3, self._get_timestamp_called)

    def test_update(self):
        def fake_list_base_images(ds_path, datastore):
            return {'unexplained_images': [],
                    'originals': self.images}

        def fake_age_and_verify_cached_images(context, datastore,
                                              dc_info, ds_path):
            self.assertEqual('[ds] fake-base-folder', ds_path)
        ...
        with contextlib.nested(
            mock.patch.object(self._imagecache, '_list_base_images',
                              fake_list_base_images),
            mock.patch.object(self._imagecache,
                              '_age_and_verify_cached_images',
                              fake_age_and_verify_cached_images)
